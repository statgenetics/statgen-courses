...
# python2.7
FROM python:2
WORKDIR /Users/Shared/OneDrive/Wang_Lab/src
COPY requirements.txt ./
RUN pip install pyGWAS -r requirements.txt
RUN pip install pandas
RUN pip install scipy
RUN pip install metaX
COPY . .
CMD [ "python2", "MetaXcan/software/MetaXcan.py"]
...
#USER nickluo1313
# to build: docker build -t nickluo1313/twas_docker -f twas.dockerfile .
